...
int main(int argc, char **argv)
{
    ...
    while( i < (CantidadEstructurasLeidas-1) )
    {
...
int CargarProductos(const char *nombreArchivo, Producto_t **listado)
{
    int CantidadEstructuras_FileIn, i, CantidadEstructurasLeidas;
    FILE *File_in = NULL;
    ...
    if(*listado == NULL)
    {
        printf("La lista esta vacia. Se creara e insertara el 1er elemento de la PILA\n");
        for( i = 0; i < CantidadEstructuras_FileIn; i++ )
        {
            listado = (Producto_t **)realloc(listado,(i+1) * sizeof(Producto_t *));
